...
// (REQUIRE_INTERNAL_TOKEN=true by default) — not a user-login JWT, so there's nothing to
// fetch from another service first. Empty = requests against protected routes get a 403,
// which the tests below tolerate (they're checking reachability/shape, not asserting auth
// is disabled).
